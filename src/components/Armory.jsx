  ...
  return (
    <div className="space-y-6">
      ...
      {activeSubTab === 'gear' ? (
          ...
          <>
            {isFormOpen && (
                <div className="glass rounded-2xl p-6 border border-red-500/30 animation-fade-in mb-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div><label className={labelClass}>Product Link (Auto-Fill)</label><div className="flex gap-2"><input className={inputClass} value={scrapeUrl} onChange={e => setScrapeUrl(e.target.value)} placeholder="Paste URL..." /><button type="button" onClick={handleAutoFill} disabled={scraping || !scrapeUrl} className="px-3 rounded-xl bg-purple-900/30 border border-purple-500/50 text-purple-300 hover:bg-purple-900/50 transition flex items-center gap-2 disabled:opacity-50">{scraping ? <Loader2 size={16} className="animate-spin"/> : <Wand2 size={16} />}</button></div></div>
            ...
            <div className="glass rounded-2xl p-6">
                {guns.length === 0 && !isFormOpen && (<div className="p-12 text-center border border-dashed border-zinc-800 rounded-xl"><Crosshair className="mx-auto text-zinc-700 mb-3" size={48} /><p className="text-zinc-500 text-sm">The Armory is empty.</p></div>)}
                <div className="space-y-3">{guns.map((gun, idx) => { const isExpanded = expandedId === gun.id; const linkedGear = gear.filter(g => gun.gearIds && gun.gearIds.map(Number).includes(Number(g.id))); return (
                    <div key={gun.id} className={`bg-black/40 border rounded-xl overflow-hidden transition-all ${isExpanded ? 'border-red-500/50 ring-1 ring-red-500/20' : 'border-zinc-800 hover:border-zinc-700'}`}>
                        <div onClick={() => toggleExpand(gun.id)} className="p-4 flex flex-col md:flex-row md:items-center justify-between cursor-pointer select-none gap-4 md:gap-0"><div className="flex items-center gap-4"><div className={`w-12 h-12 rounded-lg flex items-center justify-center flex-shrink-0 ${isExpanded ? 'bg-red-900/20 text-red-500 border border-red-900/50' : 'bg-zinc-800 text-zinc-500'}`}><Crosshair size={20} /></div><div className="min-w-0"><h4 className="text-sm font-bold text-zinc-200 truncate">{gun.name}</h4><div className="flex flex-wrap items-center gap-2 mt-0.5"><p className="text-[10px] text-zinc-500 uppercase tracking-wider">{gun.caliber || 'N/A'} • {gun.manufacturer} {gun.model}</p>{gun.ownerName && (<span className="hidden sm:flex items-center gap-1 text-[9px] px-1.5 py-0.5 rounded bg-zinc-900 border border-zinc-700 text-zinc-500"><User size={10} /> {gun.ownerName}</span>)}</div></div></div><div className="flex items-center gap-4 self-end md:self-auto"><div className="text-right"><span className="block text-[9px] text-zinc-500 uppercase tracking-wider">Rounds</span><span className="block text-xs font-mono font-bold text-emerald-500">{gun.roundCount}</span></div>{isExpanded ? <ChevronUp size={18} className="text-zinc-500" /> : <ChevronDown size={18} className="text-zinc-500" />}</div></div>
                        {isExpanded && (<div className="px-4 pb-4 pt-4 border-t border-zinc-800/50 bg-black/20"><div className="flex flex-col md:flex-row gap-6">{gun.imageUrl && (<div className="w-full md:w-1/3 max-w-sm flex-shrink-0"><div className="rounded-lg overflow-hidden border border-zinc-800 bg-black/50 shadow-lg h-48 md:h-full min-h-[160px] relative group"><img src={gun.imageUrl} alt={gun.name} className="w-full h-full object-cover" /></div></div>)}<div className="flex-1 flex flex-col justify-between"><div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-4 text-[11px] mb-4"><div><span className="text-zinc-500 block uppercase tracking-wider text-[9px] mb-0.5">Platform</span><span className="text-zinc-300 font-medium">{PLATFORMS.find(p=>p.value===gun.platform)?.label || gun.platform}</span></div><div><span className="text-zinc-500 block uppercase tracking-wider text-[9px] mb-0.5">Twist Rate</span><span className="text-zinc-300 font-medium">{gun.specs.twistRate || '--'}</span></div><div><span className="text-zinc-500 block uppercase tracking-wider text-[9px] mb-0.5">Barrel</span><span className="text-zinc-300 font-medium">{gun.specs.barrelLength ? `${gun.specs.barrelLength} in` : '--'}</span></div><div className="col-span-2 md:col-span-3 border-t border-zinc-800/50 pt-2 mt-1"><span className="text-zinc-500 block uppercase tracking-wider text-[9px] mb-1">Attached Gear</span>{linkedGear.length > 0 ? (<div className="flex flex-wrap gap-2">{linkedGear.map(g => (<span key={g.id} className="px-2 py-0.5 rounded bg-zinc-900 border border-zinc-700 text-zinc-300 text-[10px] shadow-sm">{g.name}</span>))}</div>) : <span className="text-zinc-600 italic text-[10px]">None linked.</span>}</div><div className="col-span-2 md:col-span-3 pt-2"><span className="text-zinc-500 block uppercase tracking-wider text-[9px] mb-0.5">Notes</span><span className="text-zinc-400 italic leading-relaxed">{gun.specs.notes || 'No build notes recorded.'}</span></div></div>{canEdit && (<div className="flex gap-3 pt-4 mt-auto border-t border-zinc-800/50 md:border-none md:pt-0"><button onClick={(e) => { e.stopPropagation(); handleEdit(gun); }} className="px-3 py-1.5 rounded-full bg-black/60 border border-slate-700 text-slate-300 text-[10px] hover:text-white hover:bg-slate-800 transition flex items-center gap-1"><Edit size={12}/> Edit Specs</button>{verifyDeleteId === gun.id ? (<div className="flex items-center gap-2 animate-in fade-in zoom-in duration-200"><button onClick={(e) => { e.stopPropagation(); handleDelete(gun.id); }} className="px-3 py-1.5 rounded-full bg-red-600 text-white text-[10px] font-bold hover:bg-red-500 transition">Confirm Delete</button><button onClick={(e) => { e.stopPropagation(); setVerifyDeleteId(null); }} className="px-3 py-1.5 rounded-full bg-zinc-800 text-zinc-400 text-[10px] hover:bg-zinc-700 transition">Cancel</button></div>) : (<button onClick={(e) => { e.stopPropagation(); setVerifyDeleteId(gun.id); }} className="px-3 py-1.5 rounded-full bg-black/60 border border-red-900/40 text-red-400 text-[10px] hover:text-red-300 hover:bg-red-900/20 transition flex items-center gap-1"><Trash2 size={12}/> Decommission</button>)}</div>)}</div></div></div>)}</div>);
                    })}
                </div>
            </div>
          </>
