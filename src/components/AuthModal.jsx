    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm p-0 md:p-4 pt-[env(safe-area-inset-top)]">
      <div className={`bg-[#0f0f10] border-zinc-800 md:border shadow-2xl overflow-hidden flex flex-col md:flex-row 
        ...
        <div className={`bg-black/40 p-6 flex flex-col relative border-b border-zinc-800 md:border-b-0 md:border-r ${isAdmin ? "w-full md:w-[30%] shrink-0" : "w-full flex-1"}`}>
          {canClose && !isAdmin && (<button onClick={onClose} className="absolute top-4 right-4 p-2 bg-[#1a1a1a] rounded-full text-zinc-400 hover:text-white border border-zinc-800"><X size={16} /></button>)}
            ...
            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar pb-20 md:pb-4">
                  ...
                  <div className="space-y-6">
                     <div className="bg-zinc-900/30 rounded-xl p-4 border border-zinc-800/60">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-black/20 border border-zinc-800 rounded-lg mb-4">
                            <div className="flex items-start gap-3"><div className="p-2 rounded-lg bg-zinc-800 text-zinc-500 mt-1"><Bot size={18} /></div><div><p className="text-xs font-bold text-zinc-200">AI Ballistics Expert</p><p className="text-[10px] text-zinc-500 leading-relaxed mt-1">Enable the generative AI chat assistant via OpenRouter.</p></div></div>
                        ...
                        <div className="p-4 bg-black/20 border border-zinc-800 rounded-lg space-y-4">
                            ...
                            <div>
                                ...
                                <p className="text-[9px] text-zinc-600 mt-1 italic">Leave blank to use OPENROUTER_API_KEY from .env</p>
                            </div>

                            <div className="flex gap-2 justify-end pt-2">
                                <button onClick={testAiConnection} disabled={busy} className="px-4 py-2 rounded-full bg-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-700 text-[10px] font-bold border border-zinc-700 transition flex items-center gap-2"><Zap size={12}/> Test</button>
              ...
              {activeTab === 'console' && (
                  <div className="space-y-4 h-full flex flex-col">
                      ...
                      <textarea className="w-full h-48 bg-zinc-950 border border-zinc-700 rounded-xl p-4 font-mono text-xs text-emerald-400 focus:border-red-500 focus:outline-none resize-none" placeholder="SELECT * FROM users;" value={sqlQuery} onChange={e => setSqlQuery(e.target.value)} spellCheck={false} />
                      ...
                      {sqlResult && (<div className={`p-4 rounded-xl border flex-1 overflow-hidden flex flex-col ${sqlResult.success ? 'bg-zinc-900/50 border-zinc-700' : 'bg-red-900/20 border-red-500/50'}`}><div className="flex items-center gap-2 mb-2 flex-shrink-0">{sqlResult.success ? <CheckCircle size={16} className="text-emerald-500"/> : <AlertTriangle size={16} className="text-red-500"/>}<span className="text-xs font-bold text-zinc-200">{sqlResult.success ? 'Success' : 'Error'}</span><span className="text-[10px] text-zinc-500 ml-auto">{sqlResult.message || ''}</span></div>{sqlResult.error && <pre className="text-[10px] text-red-400 whitespace-pre-wrap font-mono">{sqlResult.error}</pre>}{sqlResult.rows && (<div className="overflow-auto custom-scrollbar flex-1"><table className="w-full text-left text-[10px] text-zinc-300 border-collapse"><thead className="sticky top-0 bg-zinc-900 text-zinc-500 font-bold border-b border-zinc-700"><tr>{Object.keys(sqlResult.rows[0] || {}).map(k => <th key={k} className="p-2 whitespace-nowrap">{k}</th>)}</tr></thead><tbody>{sqlResult.rows.map((row, i) => (<tr key={i} className="border-b border-zinc-800/50 hover:bg-white/5">{Object.values(row).map((v, j) => <td key={j} className="p-2 whitespace-nowrap max-w-[200px] truncate">{v === null ? 'NULL' : String(v)}</td>)}</tr>))}</tbody></table></div>)}</div>)}
                  </div>
              )}
            </div>
          </div>
