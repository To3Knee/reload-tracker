...
//       - FIX: Full-screen responsive layout for Mobile.
//       - FIX: Maintained "Glass/Pro" look on Desktop.
//===============================================================
...
// ... (Keep PasswordInput helper exactly as is) ...
...
export default function AuthModal({
  ...
  // [STATE DECLARATIONS HERE: loginForm, newUser, etc...]
  ...
  // ... (Keep ALL useEffects and handler functions exactly the same) ...
  useEffect(() => { if (!open) return; if (isAdmin) { loadUsers(); loadSettings(); } clearMessages(); handleCancelEdit(); setVerifyActionId(null); setShowLoginPass(false); setShowRegPass(false); setShowResetPass(false); setShowApiKey(false); }, [open, currentUser, isAdmin])
  async function loadUsers() { try { const users = await listAdminUsers(); setAdminUsers(users); } catch (err) { console.log(err) } }
  async function loadSettings() { try { const data = await fetchSettings(); setSystemSettings(data); const presets = ['gemini-2.5-flash', 'gemini-2.5-pro', 'gemini-2.5-flash-lite', 'gemini-2.0-flash']; if (data.ai_model && !presets.includes(data.ai_model)) { setAiModel('custom'); setCustomModel(data.ai_model); } else { setAiModel(data.ai_model || 'gemini-2.5-flash'); setCustomModel(''); } if (data.ai_api_key) setApiKeyOverride(data.ai_api_key); } catch (err) { console.log(err) } }
  ...
  const tabBtnClass = (active) => `flex-1 text-center py-2 rounded-lg text-[10px] font-bold uppercase tracking-wider border transition whitespace-nowrap ${active ? 'bg-red-900/20 border-red-500/50 text-red-200' : 'bg-black/40 border-zinc-800 text-zinc-500 hover:text-zinc-300'}`
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm p-0 md:p-4 pt-[env(safe-area-inset-top)]">
      ...
      {/* LAYOUT FIXES:
         - Mobile: w-full h-full rounded-none (Fills screen, feels native)
         - Desktop: max-w-4xl max-h-[90vh] rounded-2xl (Centered modal)
      */}
      <div className={`bg-[#0f0f10] border-zinc-800 md:border shadow-2xl overflow-hidden flex flex-col md:flex-row 
          w-full h-full md:w-full md:max-w-4xl md:h-auto md:max-h-[90vh] 
          rounded-none md:rounded-2xl relative`}>
        
        {canClose && (
            <button onClick={onClose} className="absolute top-4 right-4 z-50 p-2 bg-[#1a1a1a] rounded-full text-zinc-400 hover:text-white hover:bg-red-900/50 border border-transparent md:border-zinc-800 transition shadow-lg">
                <X size={18} />
            </button>
        )}

        {/* LEFT PANEL */}
        <div className={`bg-black/40 p-6 flex flex-col relative border-b border-zinc-800 md:border-b-0 md:border-r ${isAdmin ? "w-full md:w-[35%] shrink-0" : "w-full flex-1"}`}>
          ...
          <div className="mb-6 flex flex-col items-center md:items-start text-center md:text-left border-b border-zinc-800/50 pb-6">
            ...
            <div>
                <h2 className="text-xl font-black text-white tracking-wide">RELOAD <span className="text-zinc-600">TRACKER</span></h2>
                <p className="text-[10px] text-zinc-500 font-medium uppercase tracking-[0.2em] mt-1">System Access Control</p>
            </div>
          </div>
          ...
          {currentUser && (
              <div className="bg-zinc-900/50 rounded-xl p-3 md:p-4 border border-zinc-800 mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-sm font-semibold text-zinc-200">{currentUser.username}</p><p className="text-[10px] text-zinc-500 uppercase">{currentUser?.role === ROLE_ADMIN ? 'Reloader (Admin)' : 'Shooter (Read-only)'}</p></div>
                </div>
              </div>
          )}
        ...
        {isAdmin && (
          <div className="flex-1 flex flex-col min-h-0 bg-gradient-to-br from-[#121214] to-[#0a0a0a]">
            ...
            <div className="flex-shrink-0 border-b border-zinc-800 p-4 bg-[#0f0f10]/95 backdrop-blur z-10">
                <div className="flex gap-2">
                    <button onClick={() => setActiveTab('manage')} className={tabBtnClass(activeTab === 'manage')}>Users</button>
                    <button onClick={() => setActiveTab('reset')} className={tabBtnClass(activeTab === 'reset')}>Passwords</button>
                    <button onClick={() => setActiveTab('system')} className={tabBtnClass(activeTab === 'system')}>Systems</button>
                </div>
            </div>
                  ...
                  <div className="space-y-6">
                     <div className="bg-zinc-900/30 rounded-xl p-4 border border-zinc-800/60">
                        ...
                        <div className="p-4 bg-black/20 border border-zinc-800 rounded-lg space-y-4">
                            <div>
                                <label className={labelClass}>AI Model <span className={subLabelClass}>(Choose '001' versions for stability)</span></label>
                                <div className="relative">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <input type={showApiKey ? "text" : "password"} className={`${inputClass} pr-10`} placeholder="Use System Env Var" value={apiKeyOverride} onChange={e => setApiKeyOverride(e.target.value)} />
                                     <button type="button" onClick={() => setShowApiKey(!showApiKey)} className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-zinc-300 transition z-10">{showApiKey ? <EyeOff size={14} /> : <Key size={14} />}</button>
                                </div>
